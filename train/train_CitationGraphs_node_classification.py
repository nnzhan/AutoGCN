...
def train_epoch(model, optimizer, device, graph, nfeat, efeat, norm_n, norm_e, train_mask, labels, epoch):
    ...
    if type(model).__name__=='APGCNNet':
        logits, steps, reminders = model(graph, nfeat, efeat, norm_n, norm_e)
        loss = model.loss(logits[train_mask], labels[train_mask], steps[train_mask], reminders[train_mask])
    else:
...
def evaluate_network(model, graph, nfeat, efeat, norm_n, norm_e, mask, labels, epoch):
    model.eval()
    epoch_test_loss = 0
    epoch_test_acc = 0
    nb_data = 0
    with torch.no_grad():
        if type(model).__name__ == 'APGCNNet':
            logits, steps, reminders = model(graph, nfeat, efeat, norm_n, norm_e)
            loss = model.loss(logits[mask], labels[mask], steps[mask], reminders[mask])
        else:
            logits = model.forward(graph, nfeat, efeat, norm_n, norm_e)
