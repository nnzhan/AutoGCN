...
class MoNet(nn.Module):
    def __init__(self, net_params):
        ...
        for _ in range(n_layers - 1):
            self.layers.append(GMMLayer(hidden_dim, out_dim, F.relu, dim, kernel, aggr_type,dropout, graph_norm, batch_norm, residual))
    ...
    def forward(self, g, h, e, snorm_n, snorm_e):
        ...
        pseudo = [[1 / np.sqrt(g.in_degree(us[i]) + 1), 1 / np.sqrt(g.in_degree(vs[i]) + 1)] for i in
