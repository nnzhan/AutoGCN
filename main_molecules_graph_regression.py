...
def main():    
    ...
    for i in range(params['runs']):
        ...
        val_ma, test_ma = train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs)
        val_mae.append(val_ma.item())
        test_mae.append(test_ma.item())
    val_mae = np.array(val_mae)
    test_mae = np.array(test_mae)

    print('\n')
    print('-'*89)
    print('Val F1: {:.3f} ± {:.3f}, Test F1: {:.3f} ± {:.3f}'.format(val_mae.mean(),val_mae.std(),test_mae.mean(), test_mae.std()))
